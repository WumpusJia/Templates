...
\title{$\text{\Huge{{\setmainfont{UnPilgi} Elders}}}_{\text{\setmainfont{UnPilgi} V3.0}}$}
\author{$\text{\setmainfont{UnPilgi} Wumpus}$}
\date{2016.10}
...
\subsection{二元一次方程解个数}
\lstinputlisting{../数学/二元一次方程解个数.cpp}
\subsection{FFT}
...
\subsection{Catalan}
\lstinputlisting{../数学/Catalan.cpp}
\subsection{线性规划}
\lstinputlisting{../数学/线性规划_uncheck.cpp}
% % % % % % % % % % % % % % % % % % % % % %
\section{数据结构}
...
\subsection{CDQ and 整体二分}
\lstinputlisting{../数据结构/CDQ_整体二分.cpp}
\subsection{树的重心}
...
\subsection{树的计数}
\lstinputlisting{../记不住的/树的计数.cpp}
\subsection{压位求含有i位的和}
\lstinputlisting{../记不住的/压位求含有i位的和.cpp}
\subsection{凸包上最大多边形面积}
\lstinputlisting{../记不住的/凸包上最大多边形面积.cpp}
% % % % % % % % % % % % % % % % % % % % % %

\section{套路}
\subsection{倍增树的直径}
\lstinputlisting{../套路/倍增维护树的直径.cpp}
\subsection{平面点哈曼顿距离}
\lstinputlisting{../套路/一群点的哈曼顿距离.cpp}
\subsection{求和的乘积最小}
\lstinputlisting{../套路/和的乘积最小.cpp}
\subsection{区间数的种类数(强制在线)}
\lstinputlisting{../套路/统计区间种类数(强制在线).cpp}
\subsection{矩形面积并}
\lstinputlisting{../套路/矩形面积并.cpp}
\subsection{骨牌覆盖方案数}
\lstinputlisting{../套路/多米诺骨牌覆盖方案数.cpp}
\subsection{补图上的bfs}
\lstinputlisting{../套路/补图上的bfs.cpp}
\subsection{点有限制，取区间}
\lstinputlisting{../套路/m个带权值区间n个点有限制,求最大权值.cpp}
\subsection{1e11的phi}
\lstinputlisting{../套路/1e11的phi.cpp}
% % % % % % % % % % % % % % % % % % % % % %
